...
class SqoopOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        :param extra_export_options: Extra export options to pass as dict.
            If a key doesn't have a value, just pass an empty string to it.
            Don't include prefix of -- for sqoop options.
        :param out_dir: Directory where avro schema gets generated
        :param avro_target_file: File where avro schema will be placed in HDFS
        """
        ...
        self.avro_target_file = avro_target_file

        if self.out_dir is None:
            raise Exception("out_dir must have a value")
        if self.avro_target_file is None:
            raise Exception("Avro target directory must be specified")
    ...
    def execute(self, context):
        ...
        if self.cmd_type == 'export':
            ...
            local_file = os.path.join(self.out_dir, self.table.replace(".", "_") + '.avsc')
            self.hook.copy_file(local_file, self.avro_target_file)
        else:
